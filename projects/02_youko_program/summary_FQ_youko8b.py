...
# ------------------------------------------------------------------
def step_draft(model, tokenizer, key_points, target_length, attempt, feedback=None, previous_summary=None):
    if attempt == 1:
        # 初回生成用プロンプト
        prompt = (
            "## 指示\n"
        ...
        prompt = (
            ...
            "3. **出力の浄化**: 要約文以外の挨拶、説明、タイトル等は一切出力しないこと。\n\n"
            f"## 重要なポイント\n{key_points}\n\n"
            f"## 前回の要約\n{previous_summary}\n\n"
            f"## 評価フィードバック\n{feedback}\n\n"
        )
...
#  Step 3: Judging
# ------------------------------------------------------------------
def step_judge(model, tokenizer, original_text, key_points, summary, target_length):
...
def generate_with_summary(args, model, tokenizer, dataset):
    for i, article in enumerate(dataset):
        ...
        key_points = step_extract_keypoints(model, tokenizer, original_text)
        ...
        for attempt in range(1, MAX_RETRY + 1):
            ...
            current_summary = step_draft(
                model, tokenizer, key_points, target_length, attempt, feedback, current_summary
            )
            current_len = len(current_summary)
            
            # 評価
            print("  [3] Judging...")
            eval_result = step_judge(model, tokenizer, original_text, key_points, current_summary, target_length)
            # --- スコア計算と判定準備 ---
