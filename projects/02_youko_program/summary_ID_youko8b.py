...
# ------------------------------------------------------------------
def step_draft(model, tokenizer, key_points, target_length):
    prompt = (
        "## 指示\n"
        f"以下の「重要なポイント」を素材として、{target_length}文字ぴったりの日本語の要約文を作成してください。\n"
        "## 制約条件\n"
...
#  Step 3: Judging
# ------------------------------------------------------------------
def step_judge(model, tokenizer, original_text, key_points, summary):
    # ユーザープロンプトに評価基準を埋め込む
    prompt = f"""
        ## 指示
    ...
    for i, article in enumerate(dataset):
        ...
        for attempt in range(1, MAX_RETRY + 1):
            ...
            current_summary = step_draft(
                model, tokenizer, key_points, target_length
            )
            current_len = len(current_summary)
            
            # 評価
            print("  [3] Judging...")
            eval_result = step_judge(model, tokenizer, original_text, key_points, current_summary)
            # --- スコア計算と判定準備 ---
