...
# ------------------------------------------------------------------
def step_draft(model, tokenizer, original_text, target_length, attempt, feedback=None, previous_summary=None):
    if attempt == 1:
        prompt = (
            "## 指示\n"
            f"以下の「入力文章」から、{target_length}文字程度の日本語の要約文を作成してください。\n"
            "## 制約条件\n"
            "1. **情報の限定**: 「入力文章」に含まれない情報は一切追加せず、固有名詞や数字などの事実関係を正確に保つこと。\n"
...
def step_judge(model, tokenizer, original_text, summary):
    ...
    prompt = f"""
        ## 指示
        以下の「元文書」を参照し、評価対象となる要約の品質を採点してください。
...
def generate_with_summary(args, model, tokenizer, dataset):
    for i, article in enumerate(dataset):
        ...
        for attempt in range(1, MAX_RETRY + 1):
            print(f"  [1] Drafting/Refining (Attempt {attempt}/{MAX_RETRY})...")
            
            # 生成
            current_summary = step_draft(
                model, tokenizer, original_text, target_length, attempt, feedback, current_summary
            )
