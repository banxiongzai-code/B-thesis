...
def main():
    ...
    generate_with_summary(args, client, dataset)
...
# ------------------------------------------------------------------
def call_llm(args, client, prompt):
    """
    LLMを呼び出す汎用関数
...
# ------------------------------------------------------------------
def step_extract_keypoints(args, client, text):
    ...
    return call_llm(args, client, prompt)

# ------------------------------------------------------------------
#  Step 2: Drafting / Refining (Single Turn Logic)
# ------------------------------------------------------------------
def step_draft(args, client, key_points, target_length, attempt, feedback=None, previous_summary=None):
    if attempt == 1:
        # 初回生成用プロンプト
        prompt = (
            "## 指示\n"
            f"以下の「重要なポイント」を素材として、{target_length}文字程度の日本語の要約文を作成してください。\n"
            "## 制約条件\n"
        ...
        # 修正用プロンプト（過去の文脈を全てここに詰め込む）
        prompt = (
            "## 指示\n"
            f"以下の「評価フィードバック」に基づき、{target_length}文字ぴったりの日本語の要約文を作成してください。\n\n"
            "## 制約条件\n"
            "1. **評価が高い項目の修正**: 評価が「良好」の項目は、現在の品質を維持してください。むやみに書き換えて改悪しないでください。\n"
            "2. **評価が低い項目の修正**: 評価が「要改善」の項目と、総合評価で指摘された点を重点的に修正してください。\n"
            "3. **文字数制御**: 「文字数指示」に従ってください。\n"
            "4. **出力の浄化**: 要約文以外の挨拶、説明、タイトル等は一切出力しないこと。\n\n"
            f"## 重要なポイント\n{key_points}\n\n"
            f"## 前回の要約\n{previous_summary}\n\n"
            f"## 評価フィードバック\n{feedback}\n\n"
...
#  Step 3: Judging
# ------------------------------------------------------------------
def step_judge(args, client, original_text, key_points, summary, target_length):
    ...
    result_text = call_llm(args, client, prompt)
...
#  Main Process Loop
# ------------------------------------------------------------------
def generate_with_summary(args, client, dataset):
    for i, article in enumerate(dataset):
        ...
        # --- [Step 1] Extraction & Save ---
        print("  [1] Extracting Key Points...")
        key_points = step_extract_keypoints(args, client, original_text)
        time.sleep(1)
        
        # 保存: 重要ポイント
        save_json(os.path.join(args.output_path, f"{article_id}_step1_points.json"), {
            "id": article_id,
        ...
        # --- [Step 2 & 3] Draft/Refine Loop ---
        for attempt in range(1, MAX_RETRY + 1):
            print(f"  [2] Drafting/Refining (Attempt {attempt}/{MAX_RETRY})...")
            
            # 生成
            current_summary = step_draft(
                args, client, key_points, target_length, attempt, feedback, current_summary
            )
            time.sleep(1)
            ...
            # 評価
            print("  [3] Judging...")
            eval_result = step_judge(args, client, original_text, key_points, current_summary, target_length)
            time.sleep(2)
            # --- スコア計算と判定準備 ---
            ...
            # 保存
            save_json(os.path.join(args.output_path, f"{article_id}_{attempt:02d}.json"), {
                "id": article_id,
                "attempt": attempt,
            ...
            if target_length == current_len and is_quality_pass:
                ...
                # --- フィードバック文章の構築 ---
                feedback_lines = []
                feedback_lines.append("前回の要約は以下の評価を受けました。この内容を反映して修正してください。\n")
                ...
                for key in quality_keys:
                    score = eval_result.get(key, 0)
                    reason = eval_result.get(f"{key}_reason", "理由なし")
                    
                    # 4点未満なら「要改善」、4点以上なら「維持」と明記すると効果的
                    status = "【要改善】" if score < 4 else "【良好】"
                    
                    line = f"- {key} ({score}/5点) {status}: {reason}"
                    feedback_lines.append(line)
                # フェーズ3で抽出した「総合的な修正指針」があれば追加
                overall_advice = eval_result.get("総合評価", "")
                if overall_advice and overall_advice != "なし":
                    feedback_lines.append(f"\n【総合評価】\n{overall_advice}")

                # 2. 文字数のフィードバック
                diff = current_len - target_length
                if diff == 0:
                    len_msg = "目標文字数ピッタリです。文字数を維持してください。"
                else:
                    direction = "減らして" if diff > 0 else "増やして"
                    len_msg = f"現在{current_len}文字です。目標は{target_length}文字ちょうどなので、あと{abs(diff)}文字{direction}ください。"
                
                feedback_lines.append("\n【文字数指示】")
                feedback_lines.append(len_msg)

                # 変数 feedback にまとめる（これを次の step_draft に渡す）
                feedback = "\n".join(feedback_lines)
