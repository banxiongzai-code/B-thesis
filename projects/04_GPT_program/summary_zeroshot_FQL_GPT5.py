...
def get_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        default="output_zeroshot_FQL_GPT5",
...
# ------------------------------------------------------------------
def step_draft(args, client, original_text, target_length):
    # 初回生成用プロンプト
    prompt = (
        "## 指示\n"
...
#  Step 3: Judging
# ------------------------------------------------------------------
def step_judge(args, client, original_text, summary):    
    # ユーザープロンプトに評価基準を埋め込む
    prompt = f"""
        ## 指示
...
def generate_with_summary(args, client, dataset):
    for i, article in enumerate(dataset):
        ...
        print(f"Article {article_id}, Count{j}")


        print(f"  [1] Generating...")
        # 生成
        current_summary = step_draft(
            args, client, original_text, target_length
        )
        time.sleep(1)
        ...
        # 評価
        print("  [2] Judging...")
        eval_result = step_judge(args, client, original_text, current_summary)
        time.sleep(1)
        # --- スコア計算と判定準備 ---
